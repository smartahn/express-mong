...
function promiseReturnTest(req, res) {
  ...
  Test.create({ name: 'test1', timestamp }).then(() => {
    return Test.find({ name: 'test1', timestamp })
      ...
      // populate
      .then(() => {
        return Test2.create({ name: 'sub1' }).then(sub1 => {
      ...
      .then((doc) => res.send('done')).catch(err => console.error(err));
  });
}
...
mongoose.connection.dropDatabase()
  .then(async() => {
    defineTestModels();
    await getAPI(PROMISE_TEST_API);
    await getAPI(CALLBACK_TEST_API);
    await getAPI(AUDIT_TEST_API);
